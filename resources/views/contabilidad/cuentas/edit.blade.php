...
@section('header_title', 'Editar Cuenta Contable: ' . $cuentaContable->codigo)
...
<div class="panel stagger-1" style="max-width: 600px; margin: 0 auto;">
    <span class="panel-tag">Modificar Detalles</span>
    
    <form action="{{ route('cuentas-contables.update', $cuentaContable->id) }}" method="POST" style="margin-top: 20px;">
        @csrf
        @method('PUT')
        ...
        <div style="margin-bottom: 15px;">
            ...
            <input type="text" name="descripcion" required value="{{ old('descripcion', $cuentaContable->descripcion) }}" style="width: 100%; padding: 10px; background-color: var(--bg); border: 1px solid var(--line); color: var(--text); border-radius: 6px;">
        ...
        <div style="display: flex; gap: 15px; margin-bottom: 15px;">
            ...
            <div style="flex: 1;">
                <label style="display: block; margin-bottom: 5px; font-size: 13px; color: var(--muted);">Nivel</label>
                <input type="number" name="nivel" min="2" max="6" required value="{{ old('nivel', $cuentaContable->nivel) }}" style="width: 100%; padding: 10px; background-color: var(--bg); border: 1px solid var(--line); color: var(--text); border-radius: 6px;">
        ...
        <div style="margin-bottom: 15px;">
            ...
            <select name="padre_id" style="width: 100%; padding: 10px; background-color: var(--bg); border: 1px solid var(--line); color: var(--text); border-radius: 6px;">
                ...
                @foreach($cuentasPrincipales as $padre)
                    <option value="{{ $padre->id }}" {{ old('padre_id', $cuentaContable->padre_id) == $padre->id ? 'selected' : '' }}>
        ...
        <input type="hidden" name="estado" value="{{ $cuentaContable->estado }}">
